...
class Lab3Driver(Node):
  ...
  # YOUR CODE HERE
		...
		# DOC: Code below are edits made for get_obstacle()

		# Collect the set of thetas based on the max and min angle range of the robot scan
		theta_array = np.linspace(angle_min, angle_max, num_readings)

		# Create empty lists for the front, left, and right to store values to help d
		front = []
		left = []
		right = []

		for i in range(0, num_readings):
			# Covert all the scan ranges to the y-components
			y = scan.ranges[i] * np.sin(theta_array[i])
			
			# Append points into each region based on the robot-half-width threshold
			if y > 0.19:
		...
		# Find the closest obstacle distance in each region. If no points exist in a region, default to infinity
		...
		# Determine the closest obstacle overall
		closest_obstacle = min(front_shortest, left_shortest, right_shortest)
		
		# Scale the speed at which the robot travels, based on how close it is to an obstacle
		avoid_speed = 0.2 * np.tanh(closest_obstacle)
		# Ensure the robot doesn't come to a full stop when maneuvering
		avoid_speed = max(0.05, avoid_speed)
				
		# Check for if obstacle is in the front
		if front_shortest < self.threshold + 0.45:
			# If there is more space on the left, turn left
			if left_shortest > right_shortest:
				return True, avoid_speed, -np.pi/24
			# Else, if there is more space on the right, turn right
			else:
				return True, avoid_speed, np.pi/24
  ...
  # YOUR CODE HERE
		...
		# DOC: Code below are edits made for get_twist()
		
		# Obstacle Checks - Call get_obstacle to retrieve the changes in speed and turn if an obstacle is in front
		in_front_of_obstacle, obstacle_speed, obstacle_turn = self.get_obstacle(scan)

		# If obstacle is in front
		if in_front_of_obstacle:
			# Update linear and turn speed
			t.twist.linear.x = obstacle_speed
			t.twist.angular.z = obstacle_turn
			return t
		
		# Retrieve the angle to target
		angle_to_target = atan2(self.target.point.y, self.target.point.x)		
		
		# Close enough Checks
		if self.close_enough():
			# Update the linear and turn speed if threshold is crossed
			t.twist.linear.x = 0.0
			t.twist.angular.z = 0.0
			return t
		
		# Update linear speed based on how close it is to the target
		# Note: this especially helps smoothen the robot stopping movement when approaching its target
		t.twist.linear.x = max_speed * np.tanh(self.distance_to_target())
		
		# Set linear speed to minimum if it crosses below the min_speed
		if t.twist.linear.x < min_speed:
			t.twist.linear.x = min_speed
		
		# Update turn speed based on the angle_to_target
